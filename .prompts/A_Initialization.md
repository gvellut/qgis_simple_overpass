copy /Users/guilhem/Documents/projects/___cloned/qgis_osminfo
...
discard the About menu

Settings : like OSM Info : in the QGIS setting panel : openable from the Web > Simple Overpass > Settings menu
...
Leep checkboxes for nearby / enclosing
Enable Debug

I want an additional checkbox (defautl checked) : Only include objects with tag
I want an additional text field / calendar : it is to enter a date: it will add a [date:"2025-07-25T15:30:00Z"];  for filtering the date (it does not have to include a time : if the query demands it : use beginning of the chosen date ie midnight ). If empty : do not include a date in the query
I want a filter text field : like the user can enter name or highway to include only the object with that tag or something like building=house to filter
I want a checkbox : Only center (default false). So instead of returning geoms like OSM Info : it returns only a point.
...
If Only center is chosen : replace the nearby with center isntead of geoms
